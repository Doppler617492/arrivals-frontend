  ...
  return (
    <div
      style={{
        ...
        borderRadius: 12,
        boxShadow:
          "0 8px 20px rgba(63,90,224,.08), inset 0 1px 0 rgba(255,255,255,.6)",
        padding: 14,
      }}
    >
      ...
      >
        ...
        {items.map((d, i) => {
          ...
          const scaled = Math.round(
            ((d.value || 0) / max) * (width - leftPad - rightPad)
          );
          const w = Math.max(minBar, scaled);
          return (
            <g key={i}>
              <text x={0} y={y + barH - 5} style={{ fontSize: 12, opacity: 0.85 }}>
                {d.name}
              </text>
              <rect x={leftPad} y={y} width={w} height={barH} rx={7} fill="#3f5ae0" fillOpacity={0.9} />
              <text
                x={leftPad + w + 8}
                y={y + barH - 5}
